...
Learn a mapping $f_{\theta}: \mathcal{X} \rightarrow \mathcal{Y}_{\text{map}}$, where $\mathcal{Y}_{\text{map}} \subset \mathbb{R}^{256 \times 256}$ is the space of continuous density maps. Each ground truth density map $\mathbf{D} \in \mathcal{Y}_{\text{map}}$ encodes the spatial distribution of cone photoreceptors, with pixel-wise densities scaled by a factor of 100. The total cone count is recovered via spatial integration: $C = \sum_{i,j} D_{i,j} / 100$. Models A and B adopt this formulation and minimize the pixel-wise mean squared error:
$$
L_{\text{density}}(\theta) = \frac{1}{n} \sum_{k=1}^{n} \|\mathbf{D}_k - f_{\theta}(\mathbf{x}_k)\|^2_2
$$

\textbf{Problem 2 (Direct Count Regression):}
Learn a mapping $g_{\phi}: \mathcal{X} \rightarrow \mathcal{Y}_{\text{count}} \subset \mathbb{R}^{+}$, where $\mathcal{Y}_{\text{count}}$ is the space of non-negative scalar cone counts. Ground truth counts $C_k \in \mathcal{Y}_{\text{count}}$ are derived from expert annotations. Models C and D directly regress cone counts and minimize:
$$
...
Table~\ref{tab:metrics_models} shows the quantitative performance of each model.
...
\textbf{Question 1 (State-of-the-art performance):} The baseline CoDE architecture achieves a mean absolute error of 1534.57 cones on our test set, establishing the state-of-the-art reference point. Our lightweight models improve upon this by 32-40\%, demonstrating that careful architectural design outweighs parameter count.

\textbf{Question 2 (Limitations):} The baseline model exhibits three critical limitations that hinder clinical adoption: (i) computational complexity with 2.06M parameters requiring extensive training (200 epochs) and GPU resources, (ii) large disk footprint (16 MB) limiting deployment on edge devices, and (iii) lower accuracy (MAE: 1534.57 cones) compared to our lightweight alternatives, likely due to overfitting in data-limited regimes.

\textbf{Question 3 (Lightweight performance):} Our results conclusively demonstrate that optimized, lighter architectures significantly outperform the complex baseline. Model A achieves 40\% better accuracy with 74\% fewer parameters and 60\% smaller model size, while Model D—the most compact variant—still exceeds baseline performance with a 77\% reduction in disk footprint (3.6 MB vs 16 MB).
