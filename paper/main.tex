...
This work directly addresses the three core research questions posed initially:

\textbf{RQ1 (Lightweight vs. State-of-the-art):} Yes, lightweight architectures with fewer than 600K parameters can not only achieve comparable accuracy but actually surpass state-of-the-art models in data-limited regimes. Our results demonstrate that Model A (538K parameters) achieves 40\% better accuracy (MAE: 921.6 vs 1534.57 cones) compared to the 2.06M-parameter baseline CoDE architecture. Similarly, Model D with only 380K parameters outperforms the baseline by 32\%. This confirms that careful architectural design and appropriate regularization through reduced capacity can outweigh raw parameter count when training data is limited (N=140 images).

\textbf{RQ2 (Density-map vs. Direct count regression):} The comparison reveals important trade-offs between the two formulations. Density-map regression (Models A and B) provides richer pixel-wise supervision (65,536 labels per image), enabling better absolute accuracy (MAE: 921.6-977.05 cones) and spatial interpretability for clinical validation. However, direct count regression (Model D) achieves superior calibration with minimal systematic bias (+15.5 cones vs. -84.5 cones for Model A) and maximum computational efficiency (77\% parameter reduction). Model C, which attempted count-level optimization on a U-Net architecture, failed to converge properly (MAE: 3884.54 cones), demonstrating that count-level loss requires careful architectural considerations. The choice between formulations depends on whether spatial density information is clinically required or only total counts are needed.

\textbf{RQ3 (Critical architectural choices):} Our experiments reveal three critical design decisions: (i) \textit{Encoder-decoder depth}: Shallow architectures (3 levels) with narrow bottlenecks (128 filters) prevent overfitting in small datasets through implicit regularization, as evidenced by stable learning curves (Fig.~\ref{fig:learning_curves}); (ii) \textit{Skip connections}: Concatenation-based skip connections (Models A, B, C) outperform residual connections for preserving spatial detail in density maps, though they become unnecessary when only counts are required (Model D); (iii) \textit{Output representations}: Decoder-based density maps enable interpretability and provide stronger training signals, but encoder-only architectures with global pooling maximize efficiency when spatial information is not required. The global-sum constraint ($\sum_{ij}\hat{D}_{ij}=\hat{C}$) acts as implicit regularization but requires pixel-wise loss (Equation~\ref{eq:loss_density}) rather than count-level loss for stable optimization.

The choice of optimal architecture depends on specific deployment requirements. Models A and B (density-map regression) are preferable when clinicians require spatial visualization of cone distribution for diagnostic interpretation and quality control. Model D (direct regression) is optimal for high-throughput screening scenarios where only total counts are needed and computational resources are constrained. Model C's failure highlights that architectural choices must align with the chosen loss formulation—count-level optimization requires either encoder-only designs or pixel-wise density supervision, not hybrid approaches.
