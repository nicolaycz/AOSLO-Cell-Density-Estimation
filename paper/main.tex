...
\author{
\IEEEauthorblockN{Nicolay Cortez}
...
$$
L_{\text{density}}(\theta) = \frac{1}{n} \sum_{k=1}^{n} \frac{1}{HW} \|\mathbf{D}_k - f_{\theta}(\mathbf{x}_k)\|^2_2
$$
where $H = W = 256$ are the spatial dimensions of the density maps.
...
The baseline model employs an Xception-based U-Net with 2.06 million trainable parameters, extensive data augmentation, and requires training for 200 epochs. The model architecture and training protocol follow the specifications detailed in the original publication.
...
\renewcommand{\arraystretch}{1.25}
    \begin{tabular}{|p{2.8cm}|p{3.2cm}|p{3.2cm}|p{3.2cm}|p{3.2cm}|p{3.2cm}|}
      \hline
\textbf{Aspect} & \textbf{Base Model (Residual Encoder-Decoder)} & \textbf{Model A (Lightweight U-Net)} & \textbf{Model B (Lightweight U-Net + Linear Regression)} & \textbf{Model C (Proposed U-Net + Global-Sum)} & \textbf{Model D (Encoder + Global Average Pooling (GAP)} \\
\hline\hline
...
(c) Model C & (d) Model D
\end{tabular}
\caption{Learning curves showing Training vs. Validation Loss over epochs. \textbf{(a) Model A} demonstrates stable stochastic convergence without a diverging gap, confirming that the lightweight architecture prevents overfitting despite the small batch size. \textbf{(b) Model B} exhibits a validation loss consistently lower than training loss, a characteristic signature of the strong Dropout regularization (0.5) effectively preventing memorization. \textbf{(c) Model C} illustrates optimization collapse (note the $1e7$ scale), indicating that scalar count-supervision alone is insufficient to guide the U-Net spatial weights. \textbf{(d) Model D} displays high initial volatility typical of direct regression without spatial priors, but eventually achieves a smooth and stable convergence.}
